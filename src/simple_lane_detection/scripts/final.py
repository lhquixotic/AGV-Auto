...
# 调用 自定义消息simple_lane_detection/object.msg
...
def image_callback(msg):
    # 将usb_cam话题转为cv2读入的img
    img = bridge.imgmsg_to_cv2(msg, "bgr8")
    for name in names:
        # img = cv2.imread(os.path.join(img_dir, name))
        # 建议传入绝对路径
        # img = cv2.imread('imgs/339215720.jpg')
        name = name.strip()
        canny = do_canny(img)
        ...
        hough_visualize = visualize_lines(img.copy(), hough)
        cv2.imwrite(os.path.join(save_dir, name[:-4]+'_hough.jpg'), hough_visualize)
        hough = filter_hough(img, hough)
        hough_visualize = visualize_lines(img.copy(), hough)
        cv2.imwrite(os.path.join(save_dir, name[:-4]+'_hough_filter.jpg'), hough_visualize)
        lines = calculate_lines(img, hough)
        #for line in lines;

        # 将lines数组传入自定义话题参数中
        point.left_cx_down = lines[0][0]
        point.left_cx_upper = lines[0][2]
        point.left_cy_down = lines[0][1]
        point.letf_cy_upper = lines[0][3]
        # # car_line_pub.publish()
        point.mid_cx_down = lines[1][0]
        point.mid_cx_upper = lines[1][2]
        point.mid_cy_down = lines[1][1]
        point.mid_cy_upper = lines[1][3]
        point.rig_cx_down = lines[2][0]
        point.rig_cx_upper = lines[2][2]
        point.rig_cy_down = lines[2][1]
        point.rig_cy_upper = lines[2][3]
        # whiel ()
        ...
        #cv2.imwrite(os.path.join(save_dir, name[:-4]+'_line.jpg'), lines_visualize)
...
if __name__ == '__main__':
    ...
    # 一直进入回调函数中
    rospy.spin()
#   car_line_pub = rospy.Publisher('/car_min_line', Point, queue_size=10)
#    car_line_pub = rospy.Publisher('/car_min_line', Point, queue_size=10)
  	# Spin the node
